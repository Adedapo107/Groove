...
/**
 * Get Spotify catalog information about albums, artists, playlists, tracks that match a
 query string.
 *
...
const getAlbum = async (req) => {
  ...
  const { data: { albums: searchAlbum } } = await getData(
    `/search?q=${query}&type=album&limit=${limit}&offset=${offset}`,
    req.cookies.access_token
  );

  const baseUrl = `${req.baseUrl}/album/${query}`;
...
const getArtist = async (req) => {
  ...
  const { data: { artists: searchArtist} } = await getData(
...
const getPlaylist = async (req) => {
  ...
  const { data: { playlists: searchPlaylist} } = await getData(
...
const getTrack = async (req, itemLimit) => {
  ...
  const { data: { tracks: searchTrack} } = await getData(
