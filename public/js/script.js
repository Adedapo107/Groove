...
const /** {HTMLElement} */ $menuToggle = document.querySelector(
    "[data-menu-toggler]"
  );
...
const ripple = function ($rippleElem) {
  $rippleElem.addEventListener("pointerdown", function (event) {
    ...
    $ripple.style.width = `${event.size}px`;
    $ripple.style.height = `${event.size}px`;
  });
};

const /** {NodeListOf<HTMLElement>} */ $rippleElems = document.querySelectorAll(
    "[data-ripple]"
  );
...
const $modalPlayerTogglers = document.querySelectorAll(
  "[data-modal-player-toggler]"
);
...
const activeSearchFilterItems = function () {
  document.querySelector('[data-search-filter].active')?.classList.remove('active');
  this.classList.add('active');
}

$searchFilterItems && addEventOnElems($searchFilterItems, 'click', activeSearchFilterItems);